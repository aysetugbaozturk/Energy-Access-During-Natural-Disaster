...
custom_sidebar_link_callback = function( select ){
  ...
  if (select == 'report'){
     ...
     var plotCalls3 = 0;
     var plotTimer3 = setInterval( function(){
       getPoints('local','test-object','test-stream', function(points){
         console.log( "The points request was successful!" );
         loadPlot3( points );
       });
   ...
   else if (select == 'batterystate'){
     var plotCalls = 0;
     var plotTimer = setInterval( function(){
       getPoints('local','data-analysis-results','battery-soc', function(points){
         console.log( "The batterystate request was successful!" );
         loadPlotBattery( points );
       });
       if( plotCalls > 20 ){
         console.log( 'Clear timer' );
         clearInterval( plotTimer );
  ...
  for ( var i = 0; i < points.length; i++){
   ...
   if( plot.highcharts().series.length > 0 ){
     ...
     plot.highcharts().addSeries({
       name: "Random Points from Python",
       data: datapoints,
       zoneAxis: 'x',
       zones: [{
         value: 8
...
function loadPlotBattery( points ){
  var plot = $('#content-batterystate');
  // Check if plot has a Highcharts element
  if( plot.highcharts() === undefined ){

    // Create a Highcharts element
    plot.highcharts( batterystate_plot_options );
  }
  // Iterate over points to place in Highcharts format
  var datapoints = [];
  for ( var i = 0; i < points.length; i++){
